  ...
  return (
    <SiteLayout>
      ...
      <PageBanner
        ...
        subtitle={page?.bannerSubtitle || 'Get in touch for fast plumbing service across Dubai'}
      />
      <section className="mx-auto max-w-7xl px-4 py-10 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div>
            <p className="text-gold font-heading font-bold uppercase tracking-[0.2em] text-sm mb-2">
              {page?.eyebrow || settings?.labels?.getInTouch || 'Get In Touch'}
            </p>
            ...
            <div className="space-y-6">
              {settings?.address && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-heading font-bold uppercase text-navy">
                      {settings?.labels?.addressLabel || 'Address'}
                    </h3>
              ...
              {phone && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-heading font-bold uppercase text-navy">
                      {settings?.labels?.phoneLabel || 'Phone'}
                    </h3>
              ...
              {settings?.email && (
                <div className="flex gap-4">
                  ...
                  <div>
                    <h3 className="font-heading font-bold uppercase text-navy">
                      {settings?.labels?.emailLabel || 'Email'}
                    </h3>
              ...
              <div className="flex gap-4">
                ...
                <div>
                  <h3 className="font-heading font-bold uppercase text-navy">
                    {settings?.labels?.hoursLabel || 'Working Hours'}
                  </h3>
                  <p className="text-slate-600">{settings?.workingHours || '24/7 Emergency Service'}</p>
                </div>
              </div>
            </div>
          </div>
          <QuoteForm
            title={page?.quoteFormTitle || 'Send Us a Message'}
            submitLabel={page?.quoteFormSubmitLabel || 'Send Message'}
            settings={settings}
          />
        </div>
      </section>
