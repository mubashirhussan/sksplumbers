...
export default async function AboutPage() {
  ...
  const stats = settings?.aboutStats?.length ? settings.aboutStats : ABOUT_STATS
  ...
  return (
    <SiteLayout>
      ...
      <PageBanner
        ...
        subtitle={page?.bannerSubtitle || "Dubai’s trusted plumbing and maintenance experts"}
      />
      <section className="mx-auto max-w-7xl px-4 py-10 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="relative aspect-[4/3] overflow-hidden">
            <CmsImage
              src={page?.image || IMAGES.about}
              alt={page?.title || 'Handyman repairing pipes under a kitchen sink'}
              fill
          ...
          <div>
            <p className="text-gold font-heading font-bold uppercase tracking-[0.2em] text-sm mb-2">
              {page?.eyebrow || labels.whoWeAre || 'Who We Are'}
            </p>
      ...
      <section className="bg-slate-50 py-10 md:py-16">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-10">
          <div className="bg-white border border-slate-200 p-8">
            <h2 className="font-heading text-2xl font-extrabold uppercase text-navy mb-4">
              {page?.missionTitle || labels.missionTitle || 'Our Mission'}
            </h2>
          ...
          <div className="bg-white border border-slate-200 p-8">
            <h2 className="font-heading text-2xl font-extrabold uppercase text-navy mb-4">
              {page?.visionTitle || labels.visionTitle || 'Our Vision'}
            </h2>
            <p className="text-slate-600 leading-relaxed">
              {page?.visionText ||
